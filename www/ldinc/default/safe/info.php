...
<div class="content3 wjcont">
 ...
 <div class="jiben1">
 	<h2>个人基本信息：</h2>
    <ul>
     <li><span>登陆账号：</span><b><?=$this->user['username']?></b></li><li><span>会员编号：</span><b><?=$this->user['uid']?></b></li>
     <li><span>等级：</span><b><?=$this->user['grade']?>级</b></li><li><span>积分：</span><b><?=$this->user['score']?></b></li>
     <li><span>会员类型：</span><b><?=$this->iff($this->user['type'], '代理', '会员')?></b></li><li><span>上级代理：</span><b><?=$this->getValue("select username from {$this->prename}members where uid=?", $this->user['parentId'])?></b></li>
     <li><span>返点：</span><b><?=$this->user['fanDian']?>%</b></li><li><span>不定位返点：</span><b><?=$this->user['fanDianBdw']?>%</b></li>
     <li><span>可用资金：</span><b><?=$this->user['coin']?> 元</b></li><li><span>冻结资金：</span><b><?=$this->user['fcoin']?> 元</b></li>
     <li><span>全部资金：</span><b><?=$this->user['coin']+$this->user['fcoin']?> 元</b></li><li></li>
    </ul>
 </div>
 <div class="clear"></div>
  <div class="jiben1">
    ...
    <ul>
     ...
     <li><span>银行账号：</span><input type="text" name="account" class="text4" value="<?=preg_replace('/^.*(\w{4})$/', '***\1', $myBank['account'])?>" <?=$this->iff($flag, 'readonly')?> /></li>
     <li><span>银行户名：</span><input type="text" name="username" class="text4"  value="<?=$this->iff($myBank['username'],mb_substr($myBank['username'],0,1,'utf-8').'**')?>" <?=$this->iff($flag, 'readonly')?> /></li>
