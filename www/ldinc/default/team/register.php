...
<div class="zhuce">
 ...
 <div class="zhucel">
  <p>　　会员登录 - 为了保障用户利益，本平台特些声明如下：</br>　　请广大用户认准本娱乐平台唯一验证地址：www.xianggang6.com 以免造成不必要的损失。</p>
  <b><a href="http://www.xianggang6.com" target="_blank">会员登录</a></b>
 </div>
 <div class="zhucer">
        <?php if($args[0]){ ?>
 <form action="/index.php/user/reg" method="post" onajax="registerBeforSubmit" enter="true" call="registerSubmit" target="ajax">
  ...
  <ul>
   <li><span>用户名：</span><input type="text" maxLength=16 name="username" id="username" value="" class="text1" /></li>
   <li><span>密码：</span><input type="password" onKeyUp="value=value.replace(/[\W]/g,'')" maxLength=16 name="password" id="password"  value="" class="text1" /></li>
   <li><span>确认密码：</span><input type="password" onKeyUp="value=value.replace(/[\W]/g,'')" maxLength=16 name="cpasswd"  id="cpasswd" value="" class="text1" /></li>
   <li><span>Q  Q：</span><input type="text" onKeyUp="value=value.replace(/[\W]/g,'')" maxLength=16 name="qq" id="qq"  value="" class="text1" /></li>
   <li><span>验证码：</span><input name="vcode"  maxLength=4 type="text" id="vcode" class="text2" /><img width="89" height="32" border="0" style="cursor:pointer;" align="absmiddle" src="/index.php/user/vcode/<?=$this->time?>" title="看不清楚，换一张图片" onclick="this.src='/index.php/user/vcode/'+(new Date()).getTime()"/></li>
  </ul>
  <b><a href="#" onclick="$(this).closest('form').submit()">注册</a></b>
    ...
    <?php }?>
    <div class="service-tel"  style="margin-top:150px; margin-left:-120px;">在线电话客服：<span class="flag-img-se"></span>0063-90957 88888
</div>
 </div>
</div>
</body>
