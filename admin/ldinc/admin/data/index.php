...
<article class="module width_full">
	<header>
		...
		<form class="submit_link wz" action="/wjadmin.php/data/index/<?=$this->type?>" target="ajax" call="defaultSearch" dataType="html">
	...
	<table class="tablesorter" cellspacing="0">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<a href="/wjadmin.php/data/add/<?=$this->type?>/<?=$var['actionNo']?>/<?=$dateString.$var['actionTime']?>" target="modal" width="340" title="添加开奖号码" modal="true" button="确定:dataAddCode|取消:defaultCloseModal">添加</a>
					<?php if($data['data']){ ?>
					<a href="/wjadmin.php/data/kj" target="ajax" data-type="<?=$typeInfo['id']?>" data-number="<?=$data['number']?>" data-time="<?=$dateString.$var['actionTime']?>" data-data="<?=$data['data']?>" onajax="setKjData" call="setKj" title="重新对没有开奖的投注开奖">开奖</a>
					<?php } ?>
