...
<article class="module width_full">
    <header>
    	<h3 class="tabs_involved">兑换订单
            <div class="submit_link wz">
            	<form action="/wjadmin.php/Score/pointList" target="ajax" call="defaultSearch" dataType="html">
	...
	<tbody> 
		...
		<tr> 
			...
			<td>
            	<?php if($state!="完成"&&$var['enable']){?><a href="/wjadmin.php/Score/pointState/<?=$var['id']?>/<?=$var['state']?>" target="ajax" call="pointHandle" dataType="json"><?=$statenext?></a> | <?php }?>
                
				<?php if($statenext!="【完成】"&&$state!="完成"){
					if($var['enable']){?>
                <a href="/wjadmin.php/Score/pointEnable/<?=$var['id']?>/<?=$var['enable']?>" target="ajax" call="pointHandle" dataType="json">取消</a> |
				<?php }else{?>
                <a href="/wjadmin.php/Score/pointEnable/<?=$var['id']?>/<?=$var['enable']?>" target="ajax" call="pointHandle" dataType="json">重启</a> |
                <?php }
				}?>
                
                <a href="/wjadmin.php/Score/pointDel/<?=$var['id']?>" target="ajax" call="pointHandle" dataType="json">删除</a>
            </td> 
		</tr>
		<?php }else{ ?>
			<tr>
