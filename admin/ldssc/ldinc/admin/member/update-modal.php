...
?>
<div>
<form action="/wjadmin.php/member/userUpdateed" target="ajax" method="post" call="userDataSubmitCode" onajax="userDataBeforeSubmitCode" dataType="html">
	...
	<table cellpadding="2" cellspacing="2" class="popupModal">
		...
		<tr>
            ...
            <td><label><input type="radio" value="1" name="enable" <?php if($userData['enable']) echo 'checked="checked"'?>/>开启</label>&nbsp;&nbsp;<label><input type="radio" value="0" name="enable" <?php if(!$userData['enable']) echo 'checked="checked"'?>/>关闭</label></td>
        </tr>
		<tr>
			<td class="title">帐号冻结：</td>
            <td><input type="text" name="passWrong" value="<?=$userData['passWrong']?>" />&nbsp;<span class="spn9">登陆密码错误次数改0解除</span></td>
		</tr>
		<tr>
