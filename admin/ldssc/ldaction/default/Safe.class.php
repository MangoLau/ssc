...
class Safe extends WebLoginBase{
	...
	public final function setCBAccount(){
		...
		if(!$para['admin']) unset($para['admin']);
		if(count($_POST) != 5) throw new Exception('参数数量出错');
		if(!isset($_POST['bankId']) || !isset($_POST['account']) || !isset($_POST['username']) || !isset($_POST['coinPassword'])) throw new Exception('参数设置出错');
//	
	$sql = "select `account` from {$this->prename}member_bank where `account` = '".$para['account']."'";
	$myData = $this->getValue($sql);
	if($myData) throw new Exception('该'.$myData.'银行账号已经使用');
		// 更新用户信息缓存
		$this->freshSession();
		if(md5($para['coinPassword'])!=$this->user['coinPassword']) throw new Exception('资金密码不正确');
		unset($para['coinPassword']);
		
		$field_list = array('bankId', 'username', 'account');
		foreach ($para as $k => $v) {
