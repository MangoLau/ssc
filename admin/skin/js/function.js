...
//任选
function tzSscRx(){
	var code=[], selCount=0, len=0, codeLen=parseInt(this.attr('length'));
	...
	if(codeLen>selCount) throw('请选'+codeLen+'位');
	var codes=code.join(',').split(",");
	codes=codes.filter(function(code){
		return code.indexOf('-')==-1;
	}).join(',');
	var codeArr=combine(codes.split(","), codeLen);
	var lens;
	//遍历组合
	for(i=0;i<codeArr.length;i++){
		lens=1;
		codeArr[i].join(',').split(',').forEach(function(v,j){
			lens*=v.split("").length;
		});
	    len+=lens;
	}
...
function tzSscHhzxInput(){
	...
	var o={"前":[16,17],"中":[174,175],"后":[19,20],"任选":[22,23]};
